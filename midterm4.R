...
if (k==1){
  ...
  b<-c()
  for(i in 1:B){
 b[i]<-sum(sample(1:6,2,replace=F))
}
return(b)
}
else {stop("Error: Invalid k")}
}
#### String manipulation
...
even<-paste("even",seq(1,1000)*2,sep="")
...
updateDate <- function(dates, old.yr) {
  old.yr.locate <- grep(old.yr, dates)
  new.dates <- gsub(old.yr, '2015', dates[old.yr.locate])
  return(new.dates)
}
