...
n.speeches <- length(breaks[-length(breaks)])
...
speechYr <- as.numeric(substring(tempDates,nchar(tempDates)-3,nchar(tempDates)))
...
speechesL <- list()
breaks <- c(breaks[-length(breaks)], length(speeches)-1)
...
se<-unlist(strsplit(se,"[[:punct:][:blank:]]+"))  
...
wordMat = matrix(unlist(wordVecs), ncol=length(wordVecs), byrow=F)
